...
int parse_subset_as_temp(struct metreport* m, struct bufr2tac_subset_state* s, struct bufr_subset_sequence_data* sq, char* err)
{
    ...
    if ((r = (struct temp_raw_data*)calloc(1, sizeof(struct temp_raw_data))) == NULL) {
        snprintf(err, ERR_SIZE, "bufr2tac: parse_subset_as_temp(): Cannot allocate memory for raw data");
        return 1;
    }

    if ((w = (struct temp_raw_wind_shear_data*)calloc(1, sizeof(struct temp_raw_wind_shear_data))) == NULL) {
        snprintf(err, ERR_SIZE, "bufr2tac: parse_subset_as_temp(): Cannot allocate memory for raw data");
        free((void*)(r));
        return 1;
    }
    ...
    if (t->a.s1.II[0]) {
        ...
        snprintf(err, ERR_SIZE, "bufr2tac: parse_subset_as_temp(): Unknown type TEMP report");
        free((void*)(r));
        free((void*)(w));
        return 1;
    }
    ...
    met_datetime_to_YYGG(t->a.s1.YYGG, sizeof(t->a.s1.YYGG), &dtm);
    ...
    if (parse_temp_raw_data(t, r)) {
        snprintf(err, ERR_SIZE, "bufr2tac: parse_temp_raw_data(): Too much significant points");
        free((void*)(r));
        free((void*)(w));
        return 1;
    }
...
int parse_temp_raw_data(struct temp_chunks* t, struct temp_raw_data* r)
{
    ...
    for (i = 0; i < r->n; i++) {
        ...
        if (d->flags & TEMP_POINT_MASK_SURFACE) {
            pascal_to_pnpnpn(t->a.s2.lev0.PnPnPn, sizeof(t->a.s2.lev0.PnPnPn), d->p); // PoPoPo
            kelvin_to_TTTa(t->a.s2.lev0.TnTnTan, sizeof(t->a.s2.lev0.TnTnTan), d->T); // TnTnTan
            dewpoint_depression_to_DnDn(t->a.s2.lev0.DnDn, sizeof(t->a.s2.lev0.DnDn), d->T, d->Td); // DnDn
            wind_to_dndnfnfnfn(t->a.s2.lev0.dndnfnfnfn, sizeof(t->a.s2.lev0.dndnfnfnfn), d->dd, d->ff); // dndnfnfnfn
        }
        ...
        if (d->flags & TEMP_POINT_MASK_STANDARD_LEVEL) {
            ...
            if (is_over_100) {
                kelvin_to_TTTa(t->c.s2.std[isc].TnTnTan, sizeof(t->c.s2.std[isc].TnTnTan), d->T); // TnTnTan
                dewpoint_depression_to_DnDn(t->c.s2.std[isc].DnDn, sizeof(t->c.s2.std[isc].DnDn), d->T, d->Td); // DnDn
                wind_to_dndnfnfnfn(t->c.s2.std[isc].dndnfnfnfn, sizeof(t->c.s2.std[isc].dndnfnfnfn), d->dd, d->ff); // dndnfnfnfn
                // Now update Id
                ...
                if (d->T != MISSING_REAL || d->Td != MISSING_REAL || d->ff != MISSING_REAL || d->dd != MISSING_REAL) {
                    // data present
                    iscv += 1;
                }
            } else {
                kelvin_to_TTTa(t->a.s2.std[isa].TnTnTan, sizeof(t->a.s2.std[isa].TnTnTan), d->T); // TnTnTan
                dewpoint_depression_to_DnDn(t->a.s2.std[isa].DnDn, sizeof(t->a.s2.std[isa].DnDn), d->T, d->Td); // DnDn
                wind_to_dndnfnfnfn(t->a.s2.std[isa].dndnfnfnfn, sizeof(t->a.s2.std[isa].dndnfnfnfn), d->dd, d->ff); // dndnfnfnfn
                // Now update Id
                if (d->ff != MISSING_REAL && d->dd != MISSING_REAL) {
        ...
        if (d->flags & TEMP_POINT_MASK_TROPOPAUSE_LEVEL) {
            if (is_over_100) {
                ...
                kelvin_to_TTTa(t->c.s3.trop[itc].TnTnTan, sizeof(t->c.s3.trop[itc].TnTnTan), d->T); // TnTnTan
                dewpoint_depression_to_DnDn(t->c.s3.trop[itc].DnDn, sizeof(t->c.s3.trop[itc].DnDn), d->T, d->Td); // DnDn
                wind_to_dndnfnfnfn(t->c.s3.trop[itc].dndnfnfnfn, sizeof(t->c.s3.trop[itc].dndnfnfnfn), d->dd, d->ff); // dndnfnfnfn
                ...
                kelvin_to_TTTa(t->a.s3.trop[ita].TnTnTan, sizeof(t->a.s3.trop[ita].TnTnTan), d->T); // TnTnTan
                dewpoint_depression_to_DnDn(t->a.s3.trop[ita].DnDn, sizeof(t->a.s3.trop[ita].DnDn), d->T, d->Td); // DnDn
                wind_to_dndnfnfnfn(t->a.s3.trop[ita].dndnfnfnfn, sizeof(t->a.s3.trop[ita].dndnfnfnfn), d->dd, d->ff); // dndnfnfnfn
        ...
        if (d->flags & TEMP_POINT_MASK_MAXIMUM_WIND_LEVEL) {
            if (is_over_100) {
                ...
                wind_to_dndnfnfnfn(t->c.s4.windx[iwxc].dmdmfmfmfm, sizeof(t->c.s4.windx[iwxc].dmdmfmfmfm), d->dd, d->ff); // dndnfnfnfn
                ...
                wind_to_dndnfnfnfn(t->a.s4.windx[iwxa].dmdmfmfmfm, sizeof(t->a.s4.windx[iwxa].dmdmfmfmfm), d->dd, d->ff); // dndnfnfnfn
                for (j = i + 1; j < r->n; j++) {
                    if (r->raw[j].ff != MISSING_REAL) {
        ...
        if (d->flags & (TEMP_POINT_MASK_SIGNIFICANT_TEMPERATURE_LEVEL | TEMP_POINT_MASK_SIGNIFICANT_HUMIDITY_LEVEL)) {
            if (is_over_100) {
                ...
                kelvin_to_TTTa(t->d.s5.th[itd].TnTnTan, sizeof(t->d.s5.th[itd].TnTnTan), d->T); // TnTnTan
                dewpoint_depression_to_DnDn(t->d.s5.th[itd].DnDn, sizeof(t->d.s5.th[itd].DnDn), d->T, d->Td); // DnDn
                ...
                kelvin_to_TTTa(t->b.s5.th[itb].TnTnTan, sizeof(t->b.s5.th[itb].TnTnTan), d->T); // TnTnTan
                dewpoint_depression_to_DnDn(t->b.s5.th[itb].DnDn, sizeof(t->b.s5.th[itb].DnDn), d->T, d->Td); // DnDn
        ...
        if (d->flags & TEMP_POINT_MASK_SIGNIFICANT_WIND_LEVEL) {
            if (is_over_100) {
                ...
                wind_to_dndnfnfnfn(t->d.s6.wd[iwd].dndnfnfnfn, sizeof(t->d.s6.wd[iwd].dndnfnfnfn), d->dd, d->ff); // dndnfnfnfn
                ...
                wind_to_dndnfnfnfn(t->b.s6.wd[iwb].dndnfnfnfn, sizeof(t->b.s6.wd[iwb].dndnfnfnfn), d->dd, d->ff); // dndnfnfnfn
