...
char* m_to_hh(char* target, size_t lmax, double h)
{
    ...
    if (ih <= 1500) {
        snprintf(target, lmax, "%02d", ih / 30);
    } else if (ih <= 9000) {
        if (ih < 1800) {
            strcpy(target, "50");
        } else {
            snprintf(target, lmax, "%2d", (ih / 300) + 50);
        }
    } else if (ih <= 21000) {
        snprintf(target, lmax, "%2d", (ih / 500) + 50);
    } else {
        strcpy(target, "89");
    }
    return target;
...
char* vism_to_VV(char* target, size_t lmax, double V)
{
    if (V < 100.0) {
        ...
        snprintf(target, lmax, "%02d", (int)(V + 0.1) / 100);
    } else if (V < 6000.0) {
        snprintf(target, lmax, "50"); // this is to avoid 51-55 range
    } else if (V <= 30000.0) {
        snprintf(target, lmax, "%02d", (int)(V + 0.1) / 1000 + 50);
    } else if (V <= 70000.0) {
        snprintf(target, lmax, "%02d", (int)(V - 30000.0) / 5000 + 80);
    } else {
        strcpy(target, "89");
    }
    return target;
...
char* m_to_RR(char* target, size_t lmax, double m)
{
    ...
    if (m > 0.0 && m < 0.00065) {
        snprintf(target, lmax, "%02d", (int)(m * 10000.0 + 0.5));
    } else if (ix <= 55) {
        snprintf(target, lmax, "%02d", ix);
    } else if (ix <= 400) {
        snprintf(target, lmax, "%02d", (ix / 10) + 50);
    } else {
        strcpy(target, "99");
    }
    return target;
...
int syn_parse_x20(struct synop_chunks* syn, struct bufr2tac_subset_state* s)
{
    ...
    case 1: // 0 20 001 . Horizontal visibility
        ...
        vism_to_VV(syn->s1.VV, sizeof(syn->s1.VV), s->val);
        break;
    ...
    case 3: // 0 20 003 . Present weather
        ...
        if ((s->i - 1) == s->k_itval && (s->i - 2) == s->k_jtval) {
            ...
            if (s->itval == 0 && s->jtval == s->tw1w2) {
                ...
                secs_to_tt(syn->s3.d9.misc[syn->s3.d9.n].spsp, sizeof(syn->s3.d9.misc[syn->s3.d9.n].spsp), s->jtval);
                syn->s3.d9.n++;
    ...
    case 10: // 0 20 010 . Cloud cover (total)
        ...
        percent_to_okta(syn->s1.N, sizeof(syn->s1.N), s->val);
        break;
    ...
    case 13: // 0 20 013 . Height of base of cloud
        ...
        if (s->clayer == 0 || s->clayer == 7 || s->clayer == 5 || s->clayer == 62) // first layer or low layer is for sec1
        {
            ...
            if (syn->s3.nub[s->clayer - 1].Ns[0]) {
                m_to_hh(syn->s3.nub[s->clayer - 1].hshs, sizeof(syn->s3.nub[s->clayer - 1].hshs), s->val);
            } else {
                m_to_9h(syn->s3.nub[s->clayer - 1].hshs, sizeof(syn->s3.nub[s->clayer - 1].hshs), s->val);
            }
        }
        break;
    ...
    case 21: // 0 20 021 . Type of precipitation
        ...
        if (syn->s0.A1[0] == '6') // for Reg VI
        {
            ...
            if (s->ival & (1 << (30 - 15)) && (s->ival & (1 << (30 - 16))) == 0 && (s->ival & (1 << (30 - 20))) == 0) {
                // Only Glaze
                strcpy(syn->s3.d9.misc[syn->s3.d9.n].SpSp, "934");
                s->SnSn = 934;
                // here we increase counter
                syn->s3.d9.n++;
            } else if (s->ival & (1 << (30 - 16)) && (s->ival & (1 << (30 - 15))) == 0 && (s->ival & (1 << (30 - 20))) == 0) {
                // Only Rime
                strcpy(syn->s3.d9.misc[syn->s3.d9.n].SpSp, "935");
                s->SnSn = 935;
                // here we increase counter
                syn->s3.d9.n++;
            } else if (s->ival & (1 << (30 - 20)) && (s->ival & (1 << (30 - 15))) == 0 && (s->ival & (1 << (30 - 16))) == 0) {
                // Only Wet snow
                strcpy(syn->s3.d9.misc[syn->s3.d9.n].SpSp, "937");
                s->SnSn = 937;
                // here we increase counter
                syn->s3.d9.n++;
            } else if (s->ival & (1 << (30 - 15)) || s->ival & (1 << (30 - 16)) || s->ival & (1 << (30 - 20))) {
                // Compound deposit
    ...
    case 23: // 0 20 023 . Other weather phenomena
        ...
        if (syn->s0.A1[0] == '6') // for Reg VI
        {
            ...
            if (s->ival & (1 << (18 - 1))) {
                ...
                // here we increase counter
                syn->s3.d9.n++;
            } else if ((s->ival & (1 << (18 - 9))) || (s->ival & (1 << (18 - 10)))) {
                // Funnel clouds
    ...
    case 54: // 0 20 054 . True direction from which clouds are moving
        ...
        if (syn->s0.A1[0] == '6') // for Reg VI
        {
            grad_to_D(aux, sizeof(aux), s->val);
            if (s->SnSn == 919 || s->SnSn == 918) {
                syn->s3.d9.misc[syn->s3.d9.n - 1].spsp[1] = aux[0];
            } else if (s->clayer == 7) {
                grad_to_D(syn->s3.Dl, sizeof(syn->s3.Dl), s->val);
            } else if (s->clayer == 8) {
                grad_to_D(syn->s3.Dm, sizeof(syn->s3.Dm), s->val);
            } else if (s->clayer == 9) {
                grad_to_D(syn->s3.Dh, sizeof(syn->s3.Dh), s->val);
            }
            syn->mask |= SYNOP_SEC3;
    ...
    case 66: // 0 20 066 . Max diameter of hailstones
        ...
        {
            m_to_RR(syn->s3.d9.misc[syn->s3.d9.n].spsp, sizeof(syn->s3.d9.misc[syn->s3.d9.n].spsp), s->val);
    ...
    case 67: // 0 20 067 . Diameter of deposit
        ...
        if (strcmp(syn->s0.A1, "1") == 0) // Only for Region I
        {
            m_to_RR(syn->s3.d9.misc[syn->s3.d9.n - 1].spsp, sizeof(syn->s3.d9.misc[syn->s3.d9.n - 1].spsp), s->val);
        }
        break;
...
int temp_parse_x20(struct temp_chunks* t, struct bufr2tac_subset_state* s)
{
    ...
    case 13: // 0 20 013 . Height of base of cloud
        ...
        m_to_h(t->b.s8.h, sizeof(t->b.s8.h), s->val);
        break;
