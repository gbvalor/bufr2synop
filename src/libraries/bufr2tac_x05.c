...
/*!
  ...
  \brief Converts true direction in degrees to D (code table 0700)
  \param [out] D Pointer to target buffer for the resulting code
  \param [in] lmax Maximum size of the target buffer
  \param [in] grad The true direction (degrees)
  \return Pointer to the resulting D string
*/
char* grad_to_D(char* D, size_t lmax, double grad)
{
    if (grad >= 22.5 && grad < 67.5) {
        strncpy_safe(D, "1", lmax);
    } else if (grad >= 67.5 && grad < 112.5) {
        strncpy_safe(D, "2", lmax);
    } else if (grad >= 112.5 && grad < 157.5) {
        strncpy_safe(D, "3", lmax);
    } else if (grad >= 157.5 && grad < 202.5) {
        strncpy_safe(D, "4", lmax);
    } else if (grad >= 202.5 && grad < 247.5) {
        strncpy_safe(D, "5", lmax);
    } else if (grad >= 247.5 && grad < 292.5) {
        strncpy_safe(D, "6", lmax);
    } else if (grad >= 292.5 && grad < 337.5) {
        strncpy_safe(D, "7", lmax);
    } else if ((grad >= 337.5 && grad <= 360.0) || (grad > 0.0 && grad < 22.5)) {
        strncpy_safe(D, "8", lmax);
    } else if (grad == 0.0) {
        strncpy_safe(D, "0", lmax);
    } else {
        strncpy_safe(D, "9", lmax);
    }
    return D;
}
...
/*!
  ...
  \brief Parse a expanded descriptor with X = 05 (Latitude and bearings)
...
int syn_parse_x05(struct synop_chunks* syn, struct bufr2tac_subset_state* s)
{
    ...
    if ((syn->s0.MMM[0] == 0) && syn->s0.LaLaLa[0] && syn->s0.LoLoLoLo[0]) {
        latlon_to_MMM(syn->s0.MMM, sizeof(syn->s0.MMM), s->lat, s->lon); // compute it
    }
