...
int parse_subset_as_synop(struct metreport* m, struct bufr2tac_subset_state* s, struct bufr_subset_sequence_data* sq, char* err)
{
    ...
    if (((s->mask & SUBSET_MASK_HAVE_WMO_ID) == 0) && ((s->mask & SUBSET_MASK_HAVE_WIGOS_ID) == 0)) {
        snprintf(err, ERR_SIZE, "bufr2tac: %s(): lack of mandatory WMO or WIGOS station identifier", __func__);
        return 1;
    }

    // Station coordinates are mandatory
    if (((s->mask & SUBSET_MASK_HAVE_LATITUDE) == 0) || 
        ((s->mask & SUBSET_MASK_HAVE_LONGITUDE) == 0))
    {
        snprintf(err, ERR_SIZE, "bufr2tac: %s(): lack of mandatory station coordinates", __func__);
        return 1;
    }

    // Date and time descriptors are mandatory
    if (((s->mask & SUBSET_MASK_HAVE_YEAR) == 0) || 
        ...
        snprintf(err, ERR_SIZE, "bufr2tac: %s(): lack of mandatory date/time descriptor in sequence", __func__);
        return 1;
    }
    ...
    // For synops from fixed land stations with no IIiii identifiers but WIGOS id, we make IIiii = '00000'
    if (strcmp(s->type_report, "AAXX") == 0 && (s->mask & SUBSET_MASK_HAVE_WMO_ID) == 0) {
    ...
    if (syn->s1.ix[0] == '/' /*&& syn->s1.ww[0] == 0*/) {
        if ((s->mask & SUBSET_MASK_HAVE_NO_SIGNIFICANT_WW) && (s->mask & SUBSET_MASK_HAVE_NO_SIGNIFICANT_W1) && (s->mask & SUBSET_MASK_HAVE_NO_SIGNIFICANT_W2)) {
    ...
    if ((syn->mask & SYNOP_SEC3) && 
        (syn->s3.SS[0] == '\0') && 
        (syn->s3.j5[0][0] || syn->s3.j5[1][0] || syn->s3.j5[2][0] || syn->s3.j5[3][0] || syn->s3.j5[4][0] || syn->s3.j5[5][0] || syn->s3.j5[6][0])) {
            snprintf(syn->s3.SS, sizeof(syn->s3.SS), "//");
    }

    if ((syn->mask & SYNOP_SEC3) && 
        (syn->s3.SSS[0] == '\0') && 
        (syn->s3.j524[0][0] || syn->s3.j524[1][0] || syn->s3.j524[2][0] || syn->s3.j524[3][0] || syn->s3.j524[4][0] || syn->s3.j524[5][0] || syn->s3.j524[6][0])) {
